...
class PostsController < ApplicationController
  ...
  def show
    full_path = "#{params[:path]}/#{params[:slug]}"
  ...
  def build_tree(posts, prefix)
    root = {}
    posts.each do |post|
      rest = post.path.sub(/^#{Regexp.escape(prefix)}\/?/, "").split("/")
      node = root
...
Rails.application.routes.draw do
  ...
  get "*path/:slug", to: "posts#show", as: :post_show,
      constraints: { path: /[^\/]+(\/[^\/]+)*/ }
  get "*path", to: "posts#tree", as: :post_tree,
      constraints: { path: /.+/ }
end
